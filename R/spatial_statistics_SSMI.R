...
sample_file  <- "../results/selected_samples_lh_for_NODDI_AIBS.csv"
probe_file   <- "../results/probes_for_analysis.csv"
img_val_file <- paste("../brain_maps/image_values_", ftd_gene, ".csv", sep="")
...
sample_adj_file <- paste("../results/sample_adjecency_knn",knn_set, "_geo", tmp, ".csv",sep="")

#location of the feather file generated by the the first python script
#GE_file <- feather("../../../raw/GE_all_pseudo_SSMI.txt.gz")

#load celltype fractions
CT_file <- "../../../raw/GE_celltype_deconvolution_KM.txt"
CT <- fread(CT_file, data.table=F, head=T)
rownames(CT) <- CT[,1]
CT <- CT[,2:ncol(CT)]
...
#output filename
ofname <- paste("../results/results_rankcor_", ftd_gene, "_", sid_str, "_knn", knn_set, "_geo",tmp, "_", dsource, ".csv", sep="")


if (dsource == "SSMI"){
  GE_file <- "../../../raw/GE_all_pseudo_SSMI.txt.gz"
  GE <- fread(paste("zcat ", GE_file), data.table=F, head=T)
  rownames(GE) <- GE[,1]
  GE <- GE[,2:ncol(GE)]
  GE <- log2(GE+1)

  if (ct_adjust){
    #add cell type adjustment
    model_str <- paste("rank(g) ~ rank(imgv)", " + ", paste(colnames(CT), collapse=" + "))
    ofname <- gsub(".csv","_CTA.csv", ofname)
  }
}

if (dsource == "CT"){
  #use cell type fractinon rather than gene expression to analyze the data
  GE_file <- "../../../raw/GE_celltype_deconvolution_KM.txt"
  GE <- fread(paste("cat ", GE_file), data.table=F, head=T)
  rownames(GE) <- GE[,1]
  GE <- t(GE[,2:ncol(GE)])
}
...
GEuse <- GE[, paste(sample_info2$well_id)]
CTuse <- CT[paste(sample_info2$well_id),] 

## select imaging data
imgv <- img_vals[paste(sample_info2$well_id),"original"]

my_df <- data.frame(imgv, sampleID=sample_info2$sampleID, CTuse, aibs.mem)
...
colnames(result) <- c("T","P","moranP","nMEM")
